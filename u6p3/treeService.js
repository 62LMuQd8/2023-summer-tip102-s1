...
// read CodePath's explanation: https://guides.codepath.org/compsci/Minimum-Depth-of-Binary-Tree
//
export class TreeService {
    ...
    minDepth() {
      ...
      function depth(root) {
        // maxDepth is different than minDepth b/c max depth always ends at a leaf node,
        // in the case of minDepth, we have to consider parent nodes with one child
        // b/c they generate false positives on min depth, ie,
        // (branch with missing subtree has depth of 0)
        //
        //       1
        //        \
        //         3
        ...
        //           7
        //
        // we cannot visit null nodes
        // so this transforms base case to edge case
        // when there is no tree, only null
        if (root === null) return 0;
        // between a parent and child node, the depth is 2
        // and a node by itself has a depth of 1 (by definition)
        // base case - we are at a leaf node, it has a depth of 1
        if (root.left === null & root.right === null) return 1;


        // we are at non-leaf node and left subtree is missing,
        // therefore, right subtree has shortest root to leaf path
        if (root.left === null) return depth(root.right) + 1;
        // we are at non-leaf node and right subtree is missing,
        // therefore, left subtree has shortest root to leaf path
        if (root.right === null) return depth(root.left) + 1;
        // default case: we are at non-leaf node,
        // parent node picks child with least depth to calc its own depth
